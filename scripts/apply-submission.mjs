...
function setOutput(name, value) {
	...
	const escaped = output.replace(/%/g, '%25').replace(/\r/g, '%0D').replace(/\n/g, '%0A');
...
function findAlumniName(alumni, github) {
	...
	return alumni.find((person) => githubKey(person.github ?? '') === key)?.nickname ?? '';
}

function upsertAlumni() {
	const nickname = requiredEnv('NICKNAME', '昵称');
	const github = normalizeGitHubUsername(requiredEnv('GITHUB_USERNAME', 'GitHub 用户名'));
	...
	const index = alumni.findIndex(
		(person) => githubKey(person.github ?? '') === githubKey(github) || person.nickname?.toLowerCase() === nickname.toLowerCase()
	);
...
function upsertProject() {
	...
	const authorGithub = normalizeGitHubUsername(optionalEnv('AUTHOR_GITHUB') || inferGitHubOwner(url));
	const authorName = authorGithub ? findAlumniName(alumni, authorGithub) || authorGithub : '未知作者';
	const languages = parseList(languagesInput);
	const index = projects.findIndex((project) => normalizeUrlKey(project.url ?? '') === normalizeUrlKey(url));
	...
	const next = {
		...
		languages: languages.length > 0 ? languages : existing.languages?.length ? existing.languages : ['Other'],
